...
export const useSettingsStore = defineStore('settings', {
  ...
  getters: {
    ...
    getEffectiveEnglishLevel(state) {
      ...
      const level = state.englishLevel === 'auto' ? state.gradeRange.max : Number(state.englishLevel);
      // 确保范围在 1-6
    ...
    // 获取年级区间标签
    gradeLabel: (state) => {
      return `${state.gradeRange.min}-${state.gradeRange.max}年级`;
    }
  },
  ...
  actions: {
    ...
    loadSettings() {
      ...
      if (settings) {
        ...
        this.gradeRange = settings.gradeRange ?? { min: 1, max: 1 };
    ...
    setGradeRange(range) {
      if (range.min >= 1 && range.max <= 6 && range.min <= range.max) {
        ...
        this.saveSettings();
        clearQuestionCache(); // 年级区间变更后清空题目缓存
      }
    },
